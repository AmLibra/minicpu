...
export class InstructionBuffer extends ComputerChipMacro {
    ...
    update(): void {
        this.readyToBeRead = this.readTimeout-- <= 0; // ready to be read when the timeout is over
    }
    ...
    private buildBufferMeshes(): Mesh {
        ...
        const rectangleSize = ((this.horizontal ? this.width : this.height) - totalSpacing) / this.size;
        const startOffset = this.horizontal
            ? this.position.x + (this.reversed ? -1 : 1) * ((rectangleSize / 2) - this.width / 2)
            : this.position.y + (this.reversed ? -1 : 1) * (rectangleSize / 2 - this.height / 2);

        let geometries = [];
        for (let i = 0; i < this.size; ++i) {
            const offset = startOffset + (this.reversed ? -1 : 1) * i * (rectangleSize + this.spacing);
            const geometry = new PlaneGeometry(this.horizontal ? rectangleSize : this.width, this.horizontal ? this.height : rectangleSize);

...
export class InstructionMemory extends ComputerChip {
    ...
    private forLoopProbability: number = 0.8;
    ...
    private typicalInstructionSequence(n: number): Queue<Instruction> {
        ...
        for (let i = 0; i < numberOfLoadOperations; ++i) {
            ...
            typicalWorkload.enqueue(Instruction.memory("LOAD", this.registerName(randomRegisterNumbers[i]), randomAddresses[i]));
        }
        ...
        for (let i = 0; i < numberOfALUOperations; ++i) {
            const randomOpcode = ISA.ALU_OPCODES[Math.floor(Math.random() * ISA.ALU_OPCODES.length)];
            ...
            typicalWorkload.enqueue(Instruction.alu(randomOpcode, result_reg, op1_reg, op2_reg));
        }
        ...
        for (let i = 0; i < instructionsLeft; ++i) {
            ...
            typicalWorkload.enqueue(Instruction.memory("STORE", this.registerName(randomResultRegister), randomAddress));
    ...
    private typicalForLoop(nPreviouslyAddedInstructions: number, n: number): Queue<Instruction> {
        ...
        const comparedTo = this.randomRegisterNumber([it]);
        const numberOfALUOperations = n - 3;

        typicalWorkload.enqueue(Instruction.aluImm("ADDI", this.registerName(it), this.registerName(it), 0));
        for (let i = 0; i < numberOfALUOperations; ++i) {
            const randomOpcode = ISA.ALU_OPCODES[Math.floor(Math.random() * ISA.ALU_OPCODES.length)];
            const result_reg = this.registerName(this.randomRegisterNumber([it, comparedTo]));
            const op1_reg = this.registerName(this.randomRegisterNumber());
            const op2_reg = this.registerName(this.randomRegisterNumber());
            typicalWorkload.enqueue(Instruction.alu(randomOpcode, result_reg, op1_reg, op2_reg));
        }

        typicalWorkload.enqueue(Instruction.aluImm("ADDI", this.registerName(it), this.registerName(it), 1));

        const branchOp = ISA.BRANCH_OPCODES[Math.floor(Math.random() * ISA.BRANCH_OPCODES.length)];
        const branchTarget = this.instructionBuffer.highestInstructionAddress() + nPreviouslyAddedInstructions + 1;
        typicalWorkload.enqueue(Instruction.branch(branchOp, this.registerName(it), this.registerName(comparedTo), branchTarget));
    ...
    private randomRegisterNumber(excepted ?: number[]): number {
        let randomRegisterNumber;
        do {
            randomRegisterNumber = 1 + Math.floor(Math.random() * (ISA.REGISTER_SIZE - 1));
